...
//  TazkiraWidget
//
//  systemMedium widget — closest match to the reference design.
//
//  Visual hierarchy (top → bottom):
//  ┌──────────────────────────────────────────────────────────────┐
//  │  الصلاة القادمة: المغرب  06:55 م                            │  ← header
//  │                                                              │
//  │  الفجر    الظهر    العصر    المغرب    العشاء                 │  ← 5 columns RTL
...
struct MediumWidgetView: View {
    ...
    private func contentView(snapshot: PrayerSnapshot) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            // Line 1: small caption "الصلاة القادمة"
            // Line 2: large bold prayer name + prominent time
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 8) {
                    Text(PrayerFormatters.formatTime(nextIsoTime))
                        .font(cairoFont(size: 14, weight: .semibold))
                        .foregroundStyle(stale ? .tertiary : .secondary)

                    Text(nextName)
                        .font(cairoFont(size: 18, weight: .bold))
    ...
    private func prayerCell(col: PrayerColumn, stale: Bool) -> some View {
        ...
        return VStack(spacing: 3) {
            Text(col.name)
                .font(cairoFont(size: 11, weight: isActive ? .bold : .regular))
